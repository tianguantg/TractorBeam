...
Widget _wrap(Widget child, {Locale locale = const Locale('zh', 'CN')}) {
  return MaterialApp(
    ...
    home: Scaffold(body: SizedBox(width: 380, height: 360, child: child)),
  );
}
...
void main() {
  ...
  group('LightweightViewState Unit Tests', () {
    test(
      'equality and hashCode include activeMode, inputDelay, and inputDelayError',
      () {
        ...
        const stateRelay = LightweightViewState(
          sessionRunning: true,
          hookReady: true,
          route: bridge.RoomRouteDto.relay,
          transport: bridge.TransportSelection.udp,
        ...
        expect(state1, isNot(equals(stateRelay)));
  ...
  group('LightweightSessionView Controls Tests', () {
    testWidgets('Header displays "← 主界面" and tapping triggers onOpenFull', (
      tester,
    ) async {
      ...
      final openFullFinder = find.byKey(
        const ValueKey('lightweight-open-full'),
      );
    ...
    testWidgets('Header displays "← Main UI" in English locale', (
      tester,
    ) async {
      final controller = _FakeLightweightController();
      var openedFull = false;
    ...
    testWidgets(
      'In-game input delay stepper can increment, decrement, and apply',
      (tester) async {
    ...
    testWidgets('Official mode disables stepper and shows notice', (
      tester,
    ) async {
      final controller = _FakeLightweightController(
        activeMode: bridge.SessionModeDto.official,
      ...
      expect(
        find.byKey(const ValueKey('lightweight-delay-minus')),
        findsNothing,
      );
      expect(
        find.byKey(const ValueKey('lightweight-delay-plus')),
        findsNothing,
      );
      expect(
        find.byKey(const ValueKey('lightweight-delay-read')),
        findsNothing,
      );
      expect(
        find.byKey(const ValueKey('lightweight-delay-apply')),
        findsNothing,
      );
    ...
    testWidgets(
      'In-game input delay read button triggers readInputDelay and resets draft',
      (tester) async {
        final controller = _FakeLightweightController(delay: 2);
    ...
    testWidgets('Displays inputDelayError icon when error is present', (
      tester,
    ) async {
